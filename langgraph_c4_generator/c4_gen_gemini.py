...
st.set_page_config(page_title="C4 Diagram Generator", page_icon="🏗️")

st.title("🏗️ C4 Diagram Generator")
st.markdown("Enter your technical specifications below, and I'll generate the Structurizr DSL for your C4 diagrams.")

# Initialize the graph and session state
if "app" not in st.session_state:
    st.session_state.app = create_graph()
    st.session_state.messages = []
    st.session_state.specification_history = ""
...
if prompt := st.chat_input("Describe your system..."):
    ...
    # Add new prompt to the specification history
    st.session_state.specification_history += "\n\n" + prompt

    # Invoke the graph with the full specification history
    with st.spinner("Generating diagrams..."):
        try:
            result = st.session_state.app.invoke({"specification": st.session_state.specification_history})
            st.session_state.dsl = result['dsl_output']
        except Exception as e:
            st.error(f"An error occurred: {e}")
            st.session_state.dsl = ""

    # Display the result
    if st.session_state.dsl:
        with st.chat_message("assistant"):
            st.markdown("Here is the generated Structurizr DSL:")
            st.code(st.session_state.dsl, language="dsl")
            st.markdown("You can save this code and use a tool like the **Structurizr CLI** to generate the visual diagrams. Feel free to add more details about your system to refine the diagram.")
